...
async def initiate_payment(
    ...
    if order.status != "pending":
        raise HTTPException(
            status_code=400, detail="Cannot initiate payment for paid/canceled order"
        )

    existing_payment = await db.execute(
        select(Payment).where(
            Payment.order_id == order_id, Payment.status == PaymentStatusEnum.pending
        )
    )
...
async def mock_payment_webhook(
    ...
    if not payment_id or new_status not in {"successful", "canceled", "refunded"}:
        raise HTTPException(
            status_code=400, detail="Missing or invalid payment_id or status"
        )
