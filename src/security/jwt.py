...
class JWTAuthManager:
    ...
    def __init__(self):
        ...
        if not self._secret_key_access or not self._secret_key_refresh:
            raise ValueError("SECRET_KEY_ACCESS and SECRET_KEY_REFRESH must be set in environment variables")

    def _create_token(self, user_id: int, secret_key: str, expires_delta: timedelta) -> str:
        expire = datetime.now(timezone.utc) + expires_delta
        payload = {"sub": str(user_id), "exp": expire}
        return jwt.encode(payload, secret_key, algorithm=self._ALGORITHM)

    def create_access_token(self, user_id: int) -> str:
        return self._create_token(user_id, self._secret_key_access, timedelta(minutes=self._ACCESS_TOKEN_EXPIRE_MINUTES))

    def create_refresh_token(self, user_id: int) -> str:
        return self._create_token(user_id, self._secret_key_refresh, timedelta(days=self._REFRESH_TOKEN_EXPIRE_DAYS))
